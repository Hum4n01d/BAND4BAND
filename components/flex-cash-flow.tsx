          ...
          steps.forEach((step, stepIndex) => {
            step.outflows.forEach((outflow) => {
              if (Math.random() < 0.4) {
                ...
                const stepElement = container.querySelector(`[data-step="${stepIndex}"]`)
                const breakdownElements = container.querySelectorAll(`[data-outflow="${step.name}-${outflow.name}"]`)
                
                if (stepElement && breakdownElements.length > 0) {
                  const stepRect = stepElement.getBoundingClientRect()
                  const randomBreakdown = breakdownElements[Math.floor(Math.random() * breakdownElements.length)]
                  ...
                  updated.push({
                    ...
                    targetY: breakdownRect.top + breakdownRect.height / 2 - rect.top,
                    progress: 0,
                    type: outflow.type
                  })
                }
    ...
    <div className="border-2 border-green-500 rounded-lg p-6 bg-black">
      ...
      <div className="relative overflow-x-auto" ref={containerRef}>
        ...
        {animatedDollars.map(dollar => (
          <div
            key={dollar.id}
            className={`absolute text-lg font-mono font-bold pointer-events-none z-10 ${
        ...
        <div className="flex items-center justify-start min-w-max space-x-16 py-8">
          {steps.map((step, stepIndex) => (
            <div key={`step-${stepIndex}`} className="flex items-center space-x-8">
              {/* Step Container */}
              <div className="flex flex-col items-center">
                {/* Step Box */}
                <div
                  className="bg-green-900 border-2 border-green-400 rounded-lg flex items-center justify-center min-w-[120px] relative"
                  style={{ 
              ...
              {step.outflows.length > 0 && (
                <div className="flex flex-col space-y-8">
                  {step.outflows.map((outflow, outflowIndex) => (
                    <div key={`outflow-${stepIndex}-${outflowIndex}`} className="flex items-center space-x-4">
                      {/* Category Label */}
                      <div className={`text-sm font-mono font-bold min-w-[120px] text-right ${
                        outflow.type === "investment" ? "text-blue-300" : "text-red-300"
                      }`}>
                        {outflow.name}
                      </div>
                      ...
                      <div className="flex flex-col space-y-2">
                        {Object.entries(outflow.breakdown).map(([breakdownName, breakdownValue], breakdownIndex) => {
                          const height = Math.max(20, Number(breakdownValue) / scaleFactor)
                          const bgColor = outflow.type === "investment" ? "bg-blue-800" : "bg-red-800"
                          const borderColor = outflow.type === "investment" ? "border-blue-300" : "border-red-300"
                          const textColor = outflow.type === "investment" ? "text-blue-200" : "text-red-200"
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="text-center px-2">
                                ...
                                <div className={`${textColor} text-xs font-mono`}>
                                  ${Number(breakdownValue).toLocaleString()}
                                </div>
                              </div>
                              
                              {/* Hover tooltip for full name if truncated */}
                              <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10">
                                {breakdownName}: ${Number(breakdownValue).toLocaleString()}
                              </div>
                            </div>
                          )
                        })}
